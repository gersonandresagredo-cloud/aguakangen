...
export default function Benefits() {
  return (
    <section id="beneficios" className="relative bg-bg-light px-5 sm:px-10 lg:px-20 py-[clamp(80px,10vw,150px)]">
      <div className="max-w-[1120px] mx-auto">
        <Reveal as="span" className="block text-[11px] font-semibold tracking-[.24em] uppercase text-aqua-deep">
          Beneficios
        </Reveal>
        <Reveal delay={80} as="h2" className="mt-[18px] max-w-[860px] text-[clamp(30px,5vw,58px)] leading-[1.02] text-ink">
          Lo que cambia cuando <span className="text-aqua">cambias tu agua.</span>
        </Reveal>
        <div className="mt-[clamp(44px,6vw,80px)] grid gap-x-[clamp(24px,4vw,56px)] gap-y-[clamp(28px,4vw,44px)]" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%,280px), 1fr))' }}>
          {ITEMS.map((it, i) => (
            <Reveal key={it.title} delay={(i % 3) * 80} data-cursor="hover" className="pt-[22px] border-t border-black/12">
              <it.icon className="w-[22px] h-[22px] text-aqua" />
              <h3 className="mt-3.5 text-[18px] tracking-[-0.02em] text-ink">{it.title}</h3>
              <p className="mt-[7px] text-sm leading-[1.55] text-ink-soft">{it.text}</p>
            </Reveal>
          ))}
        </div>
      </div>
